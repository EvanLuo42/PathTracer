...
class VBufferRTPass : public RenderPass
{
public:
    VBufferRTPass(rhi::IDevice* device, std::shared_ptr<Scene> scene, Camera* camera);

    void Setup() override;
    void Execute(rhi::ICommandEncoder* encoder, const RenderGraphResources& resources) override;
    const char* GetName() const override { return "VBufferRT"; }
